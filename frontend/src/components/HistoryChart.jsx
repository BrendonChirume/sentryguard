...
export default function HistoryChart({ history, rangeHours = 24 }) {
  const bucketByDay = rangeHours > 48;

  const points = useMemo(() => {
    const buckets = new Map();
    for (const row of history) {
      const key = bucketByDay ? Math.floor(row.timestamp / 86400) * 86400 : row.timestamp;
      buckets.set(key, (buckets.get(key) || 0) + row.total_mb);
    }
  ...
  const firstLabel = formatLabel(points[0][0], bucketByDay);
  const lastLabel = formatLabel(points[points.length - 1][0], bucketByDay);
  ...
  return (
    <div>
      ...
      <div className="flex justify-between text-[11px] text-[color:var(--c-text-4)] mt-1">
...
function formatLabel(timestampSeconds, isDay) {
